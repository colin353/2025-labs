...
$header_sidenote = "<a onclick=\"$('#newmilestoneform').show('fast')\">+ ".COLLAB_TODO_MILES2."</a>"; 
include('include/header-project.php'); ?>
<br />
<div id=newmilestoneform class=hiding><form onSubmit="return newMilestoneSubmit();">
	<span><?php echo COLLAB_TODO_MILES;?></span><input class=todolistforms type=text id=newmilestone placeholder="" />
</form>
</div>
<br />
...
<div id=milestone_<?php echo $m['todolistmilestone_id']; ?> class="milestone<?php if($pstat['next_id'] == $m['todolistmilestone_id']) echo ' milestone_current'; ?>" >
	...
	<span><?php echo COLLAB_TODO_MILES3; ?> <b><?php echo $m['todolistmilestone_name']; ?></b> <span class=sidenote><a onClick="$('#newtaskform<?php echo $m['todolistmilestone_id'];?>').show('fast')">+ <?php echo COLLAB_TODO_MILES4; ?></a></span></span>	<span class="hiding milekiller"><a href=# onClick="if(confirm('Are you for real?')) delete_milestone(<?php echo $m['todolistmilestone_id']; ?>)" >delete</a></span>
	<div id=newtaskform<?php echo $m['todolistmilestone_id'];?> class=hiding>
		<input type=checkbox /> 
		<select id=select<?php echo $m['todolistmilestone_id'];?>>
			<option value=0 ><?php echo COLLAB_TODO_MILES7; ?></option>
			<?php {
	...
	<?php 
		...
		if($t['todolist_user_id'] == 0) $tusername = COLLAB_TODO_MILES7;
		else {
	...
	<div <?php if($t['todolist_status'] == 1) echo "class=strike "; ?>id=task<?php echo $t['todolist_id']; ?>>
		...
		<span><b><?php echo $tusername; ?></b></span><span><?php echo COLLAB_TODO_MILES8; ?></span><span class=todoitem><?php echo $t['todolist_text']; ?></span>
		<span class="killer_queen hiding"><a onClick='task_delete(<?php echo  $t['todolist_id']; ?>)' href=#><?php echo COLLAB_TODO_MILES6; ?></a></span>
